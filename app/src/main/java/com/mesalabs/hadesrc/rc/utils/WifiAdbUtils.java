...
import com.mesalabs.hadesrc.root.SU;
import com.mesalabs.hadesrc.root.utils.RootUtils;
...
public class WifiAdbUtils {
    ...
    public static boolean isWifiAdbAllowed() {
        ConnectivityManager cm = (ConnectivityManager) HadesRCApp.getAppContext().getSystemService(Context.CONNECTIVITY_SERVICE);
    ...
    public static String getWifiAdbPreferenceSummary(Context context) {
        return isWifiAdbEnabled() ? getWifiIpAddress() + ":5555" : context.getString(R.string.hades_rc_disabled);
    }

    public static String getWifiIpAddress() {
        WifiManager wifimanager = (WifiManager) HadesRCApp.getAppContext().getSystemService(Context.WIFI_SERVICE);
    ...
    public static void setWifiAdb(boolean enabled) {
        if (isWifiAdbEnabled() != enabled)  {
            ...
            try {
                String sharedUserId = HadesRCApp.getAppContext()
                        .getPackageManager()
                        .getPackageInfo(HadesRCApp.getAppPackageName(), 0).sharedUserId;
